...
def get_hand_hist():
	if not os.path.exists("hist"):
		print("Error: 'hist' file not found. Please run 'set_hand_histogram.py' first to create the histogram.")
		raise FileNotFoundError("hist file not found")
...
def store_images(g_id):
	...
	while True:
		...
		# Filter out invalid contours and get the largest valid one
		valid_contours = []
		for cnt in contours:
			if len(cnt) >= 3:  # A valid contour needs at least 3 points
				try:
					area = cv2.contourArea(cnt)
					if area > 0:  # Only consider contours with positive area
	...
	print(f"Captured {pic_no} images for gesture {g_id}")
