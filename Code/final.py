...
# Try to initialize TTS engine, but make it optional
try:
	import pyttsx3
	engine = pyttsx3.init()
	...
	print(f"Warning: Text-to-speech not available ({e}). Voice features will be disabled.")
	print("To enable voice features, install eSpeak: sudo dnf install espeak (Fedora) or sudo apt-get install espeak (Ubuntu)")
	TTS_AVAILABLE = False
	engine = None
...
if not os.path.exists('cnn_model_keras2.h5'):
	print("Error: Model file 'cnn_model_keras2.h5' not found. Please train the model first.")
	exit(1)
...
def get_image_size():
	# Try to find any image file in the gestures directory
...
def get_img_contour_thresh(img):
	...
	contour_result = cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
...
def say_text(text):
	if not is_voice_on or not TTS_AVAILABLE or engine is None:
		return
	try:
